        ...
        sut.insert(feed, timestamp: timestamp) { insertionError in
            ...
            sut.retrieve { firstResult in
                sut.retrieve { secondResult in
                    ...
                    default:
                        XCTFail("Expected retrieving twice from a non empty cache to deliver the same found result with feed \(feed) and timestamp \(timestamp), but got \(firstResult) and \(secondResult) instead.")
                    }
